		...
		<Table.Body>
			{#each pageData.anggotaKeluarga as anggota, i (i)}
				<Table.Row>
					...
					<Table.Cell class="text-right">{anggota.status === 'BM' ? 'Belum Menikah' : 'Menikah'}</Table.Cell>
					<Table.Cell><Button variant="secondary" class="hover:bg-gray-300 transition-colors duration-300"><PencilIcon class="h-4"/></Button></Table.Cell>
				</Table.Row>
			{/each}
		</Table.Body>
	</Table.Root>
