...
export function drawParentNode(
	node: ParentNode,
...
export function drawRoundedRectagle(
	r: RoundedPlane2D,
	ctx: CanvasRenderingContext2D,
	fill = true,
	color = 'white',
...
export function roundedPath(c: RoundedPlane2D, ctx: CanvasRenderingContext2D) {
	...
	ctx.quadraticCurveTo(
		...
		Math.round(c.y + c.height),
		Math.round(c.x),
		Math.round(c.y + c.height - c.radius),
	);
	ctx.lineTo(Math.round(c.x), Math.round(c.y + c.radius));
	ctx.quadraticCurveTo(
		Math.round(c.x),
		Math.round(c.y),
		Math.round(c.x + c.radius),
		Math.round(c.y),
	);
	ctx.closePath();
}
